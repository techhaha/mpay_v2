...
class PostarReceiptPayment extends BasePayment implements PaymentInterface, PayPluginInterface, ChannelNotifyPayloadInterface
{
    ...
    protected array $paymentInfo = [
        ...
        'config_schema' => [
            ...
            [
                ...
                'value' => '',
                'props' => [
                    'placeholder' => '用于 receipt_watcher 区分账号，可填收款单名称或登录账号',
                ],
            ],
            [
                'type' => 'input',
                ...
                'value' => '',
                'props' => [
                    'placeholder' => '请输入星驿付收款单 renPageId，例如 209571',
                ],
                'validate' => [
            ...
            [
                ...
                'value' => '',
                'props' => [
                    'placeholder' => '请输入星驿付收款单 deptId，例如 250399',
                ],
                'validate' => [
    ...
    public function pay(array $order): array
    {
        ...
        if ($qrcode === '' && $image === '') {
            throw new PaymentException('星驿付收款单插件未配置收款码', 40200);
        }
     ...
     *
     * 支付运行时的主动查单会调用插件 query()，但星驿付收款单没有标准上游查单接口。
    ...
    public function query(array $order): array
    {
        return [
            ...
            'message' => '等待 receipt_watcher 查询星驿付流水',
        ];
    }
    ...
    {
        throw new PaymentException('星驿付收款单收款不支持接口退款', 40200);
    }
    ...
    public function notifyPayload(array $payload): array
    {
        ...
        return [
            ...
            'pay_no' => $payNo,
            'message' => 'receipt_watcher 已确认星驿付收款流水',
     ...
     * 组装收银台承接返回。
     *
     * pay_product/pay_action 用于前端和日志识别这是星驿付收款单网页流水监听场景；
    ...
    private function locatePayNoByAmount(array $record): string
    {
        ...
        if ($orders->count() > 1) {
            Log::warning(sprintf(
                '[PostarReceiptPayment] 金额流水匹配到多笔订单，按时间最近选择 amount=%s count=%d',
    ...
    private function locatePayNoByRemark(array $record): string
    {
        ...
        if ($candidates->count() > 1) {
            Log::warning(sprintf(
                '[PostarReceiptPayment] 备注流水匹配到多笔订单，按时间最近选择 remark=%s count=%d',
                $remarkCode,
                $candidates->count()
            ));
        }
     ...
     * 根据流水支付方式编码解析支付方式 ID。
     *
     * 星驿付收款单可能同时支持支付宝、微信、银联等支付方式，流水中的 pay_type
     * 用于进一步缩小金额匹配范围。
     *
     * @param array<string, mixed> $record 流水记录
     * @return int 支付方式ID
     ...
     * 获取当前收款账号对应的所有通道 ID。
     *
     * 同一个星驿付插件配置可以绑定多个支付方式通道，金额占用和流水匹配必须在同一
    ...
    private function receiptChannelIds(): array
    {
        $ids = $this->paymentChannelRepository->idsByPluginConfig(
            (string) $this->getConfig('plugin_code', 'postar_receipt'),
    ...
    private function accountKey(): string
    {
        return preg_replace(
            '/[^A-Za-z0-9_\\-]/',
            '_',
            (string) $this->getConfig('plugin_code', 'postar_receipt') . '_' . (int) $this->getConfig('api_config_id')
        ) ?: 'postar_receipt_0';
    }
     ...
     * 生成渠道交易号。
     *
     * receipt_watcher 已保证星驿付流水的 order_no 固定来自收款单 `orderId`。
     *
     * @param array<string, mixed> $record 流水记录
