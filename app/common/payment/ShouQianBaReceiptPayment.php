 ...
 *
 * 适用场景：
 * - 商户使用收钱吧这类第三方平台提供的二维码牌收款。
 * - 平台不会像官方支付 API 一样主动回调本系统。
 * - Python receipt_watcher 登录网页后台查询流水，再把归一化流水投递到 Redis 队列。
 *
 * 职责边界：
 * - 本插件负责生成收银台二维码承接参数、分配金额/备注识别信息、根据流水定位支付单。
 * - Webman 队列服务负责调用 channelNotifyPayload() 定位 pay_no，再调用 notifyPayload() 取得标准通知结果。
 * - 第三方网页登录地址、页面选择器、接口拦截规则不放在插件配置中，统一在 receipt_watcher 平台适配类维护。
 *
 * 金额口径：
...
class ShouQianBaReceiptPayment extends BasePayment implements PaymentInterface, PayPluginInterface, ChannelNotifyPayloadInterface
{
     ...
     * 插件元信息和后台配置表单。
     *
     * 配置中只保存业务所需字段。网页后台地址、页面选择器、验证码识别策略等属于
     * receipt_watcher 的平台适配逻辑，不放进这里，避免后台配置和抓取实现强耦合。
     *
    ...
    protected array $paymentInfo = [
        ...
        'config_schema' => [
            ...
            [
                ...
                'value' => '',
                'props' => [
                    'placeholder' => '用于 receipt_watcher 区分账号，可填门店号或登录账号',
                ],
            ],
            [
                'type' => 'input',
    ...
    public function pay(array $order): array
    {
        ...
        if ($qrcode === '' && $image === '') {
            throw new PaymentException('收钱吧二维码牌插件未配置收款码', 40200);
        }
     ...
     *
     * 支付运行时的主动查单会调用插件 query()，但二维码牌没有标准上游查单接口。
    ...
    public function query(array $order): array
    {
        return [
            ...
            'message' => '等待 receipt_watcher 查询收钱吧流水',
        ];
    }
    ...
    {
        throw new PaymentException('收钱吧二维码牌收款不支持接口退款', 40200);
    }
    ...
    public function notifyPayload(array $payload): array
    {
        ...
        return [
            ...
            'pay_no' => $payNo,
            'message' => 'receipt_watcher 已确认收钱吧收款流水',
     ...
     * 组装收银台承接返回。
     *
     * pay_product/pay_action 用于前端和日志识别这是二维码牌网页流水监听场景；
    ...
    private function locatePayNoByAmount(array $record): string
    {
        ...
        if ($orders->count() > 1) {
            Log::warning(sprintf(
                '[ShouQianBaReceiptPayment] 金额流水匹配到多笔订单，按时间最近选择 amount=%s count=%d',
    ...
    private function locatePayNoByRemark(array $record): string
    {
        ...
        if ($candidates->count() > 1) {
            Log::warning(sprintf(
                '[ShouQianBaReceiptPayment] 备注流水匹配到多笔订单，按时间最近选择 remark=%s count=%d',
                $remarkCode,
                $candidates->count()
            ));
        }
     ...
     * 根据流水支付方式编码解析支付方式 ID。
     *
     * 收钱吧二维码牌可能同时支持支付宝、微信、银联等支付方式，流水中的 pay_type
     * 用于进一步缩小金额匹配范围。
     *
     * @param array<string, mixed> $record 流水记录
     * @return int 支付方式ID
     ...
     * 获取当前收款账号对应的所有通道 ID。
     *
     * 同一个收钱吧插件配置可以绑定多个支付方式通道，金额占用和流水匹配必须在同一
    ...
    private function receiptChannelIds(): array
    {
        $ids = $this->paymentChannelRepository->idsByPluginConfig(
            (string) $this->getConfig('plugin_code', 'shouqianba_receipt'),
    ...
    private function accountKey(): string
    {
        return preg_replace(
            '/[^A-Za-z0-9_\\-]/',
            '_',
            (string) $this->getConfig('plugin_code', 'shouqianba_receipt') . '_' . (int) $this->getConfig('api_config_id')
        ) ?: 'shouqianba_receipt_0';
    }

    /**
     * 生成渠道交易号。
     *
     * receipt_watcher 已保证收钱吧流水的 order_no 固定来自 order_sn。
     *
     * @param array<string, mixed> $record 流水记录
