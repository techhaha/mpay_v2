...
class EpayMockChainTest extends Command
{
    ...
    private function runV1Chain(OutputInterface $output, PaymentType $paymentType): array
    {
        ...
        $submitResponse = $this->resolve(EpayV1Controller::class)->submit($this->buildRequest($submitPayload, '/submit.php'));
        ...
        $mapiResponse = $this->decodeJsonResponse(
            $this->resolve(EpayV1Controller::class)->mapi($this->buildRequest($mapiPayload, '/mapi.php'))->rawBody()
        );
        ...
        $v1CallbackAck = $this->resolve(EpayV2Controller::class)->callback(
    ...
    private function runV2Chain(OutputInterface $output, PaymentType $paymentType): array
    {
        ...
        $mockPlatformPrivateKey = (string) $context['mock_platform_private_key'];
        ...
        $submitResponse = $this->resolve(EpayV2Controller::class)->submit($this->buildRequest($submitPayload, '/api/pay/submit'));
        $submitSnapshot = $this->loadOrderSnapshot((int) $merchant->id, $submitOutTradeNo);
        ...
        $v2CallbackPayload = $this->buildV2CallbackPayload($merchant, $createBizOrder, $createPayOrder, $mockPlatformPrivateKey);
        $v2CallbackAck = $this->resolve(EpayV2Controller::class)->callback(
    ...
    private function ensureV1Context(OutputInterface $output, PaymentType $paymentType): array
    {
        ...
        $pluginConf = $this->ensurePluginConf('epay_v1', '支付链路测试-V1', [
            'gateway_url' => 'mock://epay-v1',
            'upstream_pid' => '900001',
    ...
    private function ensureV2Context(OutputInterface $output, PaymentType $paymentType): array
    {
        ...
        $pluginConf = $this->ensurePluginConf('epay_v2', '支付链路测试-V2', [
            'gateway_url' => 'mock://epay-v2',
            'upstream_pid' => '900002',
            'merchant_private_key' => $upstreamMerchantPair['private_key'],
            'platform_public_key' => $upstreamPlatformPair['public_key'],
            'mock_platform_private_key' => $upstreamPlatformPair['private_key'],
            'mock_enabled' => true,
    ...
    private function callV1Api(array $payload): array
    {
        return $this->decodeJsonResponse(
            $this->resolve(EpayV1Controller::class)->api($this->buildRequest($payload, '/api.php'))->rawBody()
        );
    }
    ...
    private function callV2JsonEndpoint(string $action, array $payload): array
    {
        ...
        return $this->decodeJsonResponse($response->rawBody());
    }
